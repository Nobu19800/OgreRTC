...
/**
*@brief シミュレーションのジョイントのクラスのコンストラクタ
* @param n 名前
* @param s シミュレーションオブジェクト
*/
ODEJointObj::ODEJointObj(std::string n, SimulationObj *s) : LinkObj(n)
{
	ms = s;
	name = n;
	type = 0;
...
* @param sz 拡大率(Z)
*/
void ODEJointObj::SetODEScale(float sx, float sy, float sz)
{
	bscale_x = sx;
	bscale_y = sy;
	bscale_z = sz;
...
* @param sz オフセット(Z)
*/
void ODEJointObj::SetODEOffset(float ox, float oy, float oz)
{
	offx = ox;
	offy = oy;
	offz = oz;
...
*/
bool ODEJointObj::openb(std::ifstream &ifs)
{
	name = ReadString(ifs);
...
* @param ofs2 ファイルストリーム
*/
void ODEJointObj::save(std::ofstream &ofs2)
{
...
* @param y ヨー
*/
void ODEJointObj::SetJointRotation(double r, double p, double y)
{
	ms->mu->lock();
	if(JointType == 0)
	{
...
* @param v 目標角度、目標位置
*/
void ODEJointObj::ControlJointAng(double v)
{
